...
perform_muscat_de_analysis <- function(sce, sample_id, celltype_id, group_id, batches, covariates, contrasts, expressed_df, assay_oi_pb = "counts", fun_oi_pb = "sum", de_method_oi = "edgeR", min_cells = 10) {
  ...
  if (sum(table(sce$sample_id, sce$group_id) %>% apply(1, function(row_oi) {
    sum(row_oi > 0)
  }) > 1) > 0) {
  ...
  de_output_tidy <- muscat::resDS(sce, res, bind = "row", cpm = FALSE, frq = FALSE) %>%
    tibble::as_tibble() %>%
...
p.adjust_empirical <- function(pvalues, tvalues, plot = FALSE, celltype = NULL, contrast = NULL) {
  ...
  ml.out <- locfdr:::locmle(zvalues_mid, xlim = c(
    mlests[1],
    b * mlests[2]
  ...
  pval_empirical <- 2 * pnorm(-abs(zval_empirical),
    mean = 0,
    sd = 1
  )
  ...
  if (plot) {
    ...
    gg_plot <- gg_plot +
      geom_line(
        ...
        color = "darkgreen",
        lwd = 1
      ) + ggtitle(paste0("Empirical distribution of z-scores\n", celltype, " : ", contrast))
  } else {
    gg_plot <- NA
  }
...
add_empirical_pval_fdr <- function(de_output_tidy, plot = FALSE) {
  ...
  de_output_tidy_new <- all_celltypes %>%
    lapply(function(cluster_id_oi, de_output_tidy) {
      all_contrasts %>%
