...
#'
#'
perform_muscat_de_analysis <- function(
  sce,
  sample_id,
  celltype_id,
  group_id,
  ...
  if (length(contrasts) != 1 | !is.character(contrasts)) {
    stop(
      "contrasts should be a character vector of length 1. See the documentation of the function for having an idea of the right format of setting your contrasts."
    )
  }
  ...
  sce <- muscat::prepSCE(
    ...
    drop = FALSE
  ) # drop all other SummarizedExperiment::colData columns ----------------- change to false
  ...
  if (
    ...
    stop(
      "One or more of your sample_ids belongs to more than one group/condition of interest. Please make sure that all sample_ids are uniquely divided over your groups/conditions."
    )
  }
  ...
  if (batches_present == TRUE & covariates_present == FALSE) {
    ...
    dimnames(design) <- list(
      ...
      c(
        ...
        make.names(colnames(design)[
          (length(levels(ei$group_id)) + 1):length(colnames(design))
        ])
      )
    )
    ...
    design <- eval(parse(
      text = paste("model.matrix(~ 0 + ei$group_id) ", sep = "")
    ))
...
#' @export
#'
get_FDR_empirical_plots <- function(
  de_output_tidy,
  cluster_id_oi,
  contrast_oi
) {
