generate_prioritization_tables_OLINK <- function(sender_receiver_info, sender_receiver_de, ligand_activities_targets_DEgenes, ligand_activities_targets_OLINK, contrast_tbl, sender_receiver_tbl, grouping_tbl, prioritizing_weights = c("de_ligand" = 1, "de_receptor" = 1, "de_ligand_OLINK" = 2, "de_receptor_OLINK" = 2, "activity_scaled" = 2, "activity_OLINK" = 1, "exprs_ligand" = 3, "exprs_receptor" = 3, "frac_exprs_ligand_receptor" = 1, "abund_sender" = 0, "abund_receiver" = 0), fraction_cutoff, abundance_data_receiver, abundance_data_sender) {
  ...
  receiver_receptor_prioritization <- sender_receiver_de %>%
    dplyr::ungroup() %>%
    dplyr::select(contrast, receiver, receptor, lfc_receptor, p_val_receptor) %>%
    dplyr::distinct() %>%
    dplyr::mutate(lfc_pval_receptor = -log10(p_val_receptor) * lfc_receptor, p_val_receptor_adapted = -log10(p_val_receptor) * sign(lfc_receptor))
  receiver_receptor_prioritization <- receiver_receptor_prioritization %>%
    dplyr::mutate(scaled_lfc_receptor = rank(lfc_receptor, ties.method = "average", na.last = FALSE) / max(rank(lfc_receptor, ties.method = "average", na.last = FALSE)), scaled_p_val_receptor = rank(desc(p_val_receptor), ties.method = "average", na.last = FALSE) / max(rank(desc(p_val_receptor), ties.method = "average", na.last = FALSE)), scaled_lfc_pval_receptor = rank(lfc_pval_receptor, ties.method = "average", na.last = FALSE) / max(rank(lfc_pval_receptor, ties.method = "average", na.last = FALSE)), scaled_p_val_receptor_adapted = rank(p_val_receptor_adapted, ties.method = "average", na.last = FALSE) / max(rank(p_val_receptor_adapted, ties.method = "average", na.last = FALSE))) %>%
    dplyr::arrange(-lfc_pval_receptor)

  receiver_receptor_prioritization_OLINK <- olink_df_receptor %>%
    rename(lfc_receptor = logFC, p_val_receptor = pval) %>%
    dplyr::ungroup() %>%
    dplyr::distinct() %>%
    dplyr::mutate(lfc_pval_receptor = -log10(p_val_receptor) * lfc_receptor, p_val_receptor_adapted = -log10(p_val_receptor) * sign(lfc_receptor))
  receiver_receptor_prioritization_OLINK <- receiver_receptor_prioritization_OLINK %>%
    dplyr::mutate(scaled_lfc_receptor = rank(lfc_receptor, ties.method = "average", na.last = FALSE) / max(rank(lfc_receptor, ties.method = "average", na.last = FALSE)), scaled_p_val_receptor = rank(desc(p_val_receptor), ties.method = "average", na.last = FALSE) / max(rank(desc(p_val_receptor), ties.method = "average", na.last = FALSE)), scaled_lfc_pval_receptor = rank(lfc_pval_receptor, ties.method = "average", na.last = FALSE) / max(rank(lfc_pval_receptor, ties.method = "average", na.last = FALSE)), scaled_p_val_receptor_adapted = rank(p_val_receptor_adapted, ties.method = "average", na.last = FALSE) / max(rank(p_val_receptor_adapted, ties.method = "average", na.last = FALSE))) %>%
    dplyr::arrange(-lfc_pval_receptor)
  receiver_receptor_prioritization_OLINK <- receiver_receptor_prioritization_OLINK %>%
    rename(scaled_lfc_receptor_OLINK = scaled_lfc_receptor, scaled_p_val_receptor_adapted_OLINK = scaled_p_val_receptor_adapted) %>%
    distinct(contrast, receptor, scaled_lfc_receptor_OLINK, scaled_p_val_receptor_adapted_OLINK)
  ...
  receiver_ligand_activity_prioritization_up <- ligand_activities_targets_DEgenes$ligand_activities %>%
    ...
    dplyr::rename(activity_up = activity, activity_scaled_up = activity_scaled) %>%
    dplyr::distinct() %>%
    dplyr::mutate(scaled_activity_scaled_up = nichenetr::scale_quantile_adapted(activity_scaled_up, outlier_cutoff = 0.01), scaled_activity_up = nichenetr::scale_quantile_adapted(activity_up, outlier_cutoff = 0.01)) %>%
  ...
  receiver_ligand_activity_prioritization_down <- ligand_activities_targets_DEgenes$ligand_activities %>%
    ...
    dplyr::select(contrast, receiver, ligand, activity, activity_scaled) %>%
    dplyr::rename(activity_down = activity, activity_scaled_down = activity_scaled) %>%
    dplyr::distinct() %>%
    dplyr::mutate(scaled_activity_scaled_down = nichenetr::scale_quantile_adapted(activity_scaled_down, outlier_cutoff = 0.01), scaled_activity_down = nichenetr::scale_quantile_adapted(activity_down, outlier_cutoff = 0.01)) %>%
  ...
  ligand_celltype_specificity_prioritization_pb <- sender_receiver_info$pb_df_group %>%
    ...
    dplyr::mutate(scaled_pb_ligand = nichenetr::scale_quantile_adapted(pb_ligand_group)) %>%
  ...
  receptor_celltype_specificity_prioritization_pb <- sender_receiver_info$pb_df_group %>%
    ...
    dplyr::mutate(scaled_pb_receptor = nichenetr::scale_quantile_adapted(pb_receptor_group)) %>%
  ...
  ligand_receptor_expressed_prioritization <- sender_receiver_info$frq_df %>%
    ...
    dplyr::ungroup() %>%
    dplyr::select(sample, group, sender, receiver, ligand, receptor, fraction_ligand, fraction_receptor) %>%
    dplyr::distinct() %>%
    ...
    dplyr::mutate(fraction_expressing_ligand_receptor = n_expressing / n_samples) %>%
  ...
  sender_abundance_prioritization <- sender_receiver_info$rel_abundance_df %>%
    ...
    dplyr::inner_join(contrast_tbl) %>%
    dplyr::ungroup() %>%
    dplyr::select(group, sender, rel_abundance_scaled_sender) %>%
    dplyr::distinct() %>%
    dplyr::arrange(-rel_abundance_scaled_sender)
  ...
  receiver_abundance_prioritization <- sender_receiver_info$rel_abundance_df %>%
    ...
    dplyr::inner_join(contrast_tbl) %>%
    dplyr::ungroup() %>%
    dplyr::select(group, receiver, rel_abundance_scaled_receiver) %>%
    dplyr::distinct() %>%
    dplyr::arrange(-rel_abundance_scaled_receiver)

  # OLINK lignand activity prioritization
  OLINK_ligand_activity_prioritization_up <- ligand_activities_targets_OLINK$ligand_activities %>%
    dplyr::ungroup() %>%
    dplyr::filter(direction_regulation == "up") %>%
    dplyr::select(contrast, ligand, activity, activity_scaled) %>%
    dplyr::rename(activity_up_OLINK = activity, activity_scaled_up_OLINK = activity_scaled) %>%
    dplyr::distinct() %>%
    dplyr::mutate(scaled_activity_scaled_up_OLINK = nichenetr::scale_quantile_adapted(activity_scaled_up_OLINK, outlier_cutoff = 0.01), scaled_activity_up_OLINK = nichenetr::scale_quantile_adapted(activity_up_OLINK, outlier_cutoff = 0.01)) %>%
    dplyr::arrange(-activity_scaled_up_OLINK)
  ...
  sample_prioritization_tbl <- sender_receiver_info$avg_df %>%
    ...
    dplyr::inner_join(grouping_tbl) %>%
    dplyr::left_join(group_prioritization_tbl %>% dplyr::distinct(group, sender, receiver, ligand, receptor, prioritization_score)) # maybe NA there if it is not considered to be expressed
  ...
  sample_prioritization_tbl <- sample_prioritization_tbl %>%
    dplyr::group_by(id) %>%
    dplyr::mutate(scaled_LR_prod = nichenetr::scaling_zscore(ligand_receptor_prod), scaled_LR_frac = nichenetr::scaling_zscore(ligand_receptor_fraction_prod), scaled_LR_pb_prod = nichenetr::scaling_zscore(ligand_receptor_pb_prod)) %>%
    dplyr::ungroup()
  ...
  ligand_activities_target_de_tbl <- ligand_activities_targets_DEgenes$ligand_activities %>%
    dplyr::inner_join(ligand_activities_targets_DEgenes$de_genes_df %>% dplyr::rename(target = gene, p_val_adj = p_adj)) %>%
    dplyr::select(contrast, receiver, ligand, activity, activity_scaled, target, ligand_target_weight, logFC, p_val, p_val_adj, direction_regulation) %>%
    dplyr::distinct()
  ...
  ligand_activities_target_de_tbl_OLINK <- ligand_activities_targets_OLINK$ligand_activities %>%
    dplyr::inner_join(ligand_activities_targets_OLINK$de_genes_df %>% dplyr::rename(target = gene)) %>%
    dplyr::select(contrast, ligand, activity, activity_scaled, target, ligand_target_weight, logFC, pval, direction_regulation) %>%
    dplyr::distinct()
...
get_ligand_activities_targets_OLINK <- function(olink_df, ligand_target_matrix, logFC_threshold = 1, pval_threshold = 0.05, top_n_target = 250, verbose = FALSE) {
  ...
  for (i in seq(length(de_output_tidy$contrast %>% unique()))) {
    ...
    de_tbl_geneset <- de_output_tidy %>%
      ...
      dplyr::filter(logFC >= logFC_threshold & pval <= pval_threshold)
    ...
    if (length(geneset_oi) > 0) {
      ...
      ligand_target_df <- ligand_activities$ligand %>%
        unique() %>%
        lapply(nichenetr::get_weighted_ligand_target_links, geneset_oi, ligand_target_matrix, top_n_target) %>%
        dplyr::bind_rows() %>%
        dplyr::mutate(contrast = contrast_oi) %>%
      ...
      ligand_activities <- ligand_activities %>%
        ...
        dplyr::mutate(activity_scaled = nichenetr::scaling_zscore(activity), direction_regulation = "up")
    } else {
