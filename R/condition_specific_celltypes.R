...
#'
#'
generate_prioritization_tables_condition_specific_celltypes_sender = function(
  ...
  receiver_receptor_prioritization = receiver_receptor_prioritization %>%
    dplyr::mutate(
      scaled_lfc_receptor = base::rank(
        ...
        max(base::rank(lfc_receptor, ties.method = "average", na.last = FALSE)),
      scaled_p_val_receptor = base::rank(
        desc(p_val_receptor),
        ties.method = "average",
        na.last = FALSE
      ) /
      ...
      scaled_lfc_pval_receptor = base::rank(
        lfc_pval_receptor,
        ties.method = "average",
        na.last = FALSE
      ) /
      ...
      scaled_p_val_receptor_adapted = base::rank(
  ...
  sender_ligand_prioritization = sender_ligand_prioritization %>%
    dplyr::mutate(
      scaled_lfc_ligand = base::rank(
        ...
        max(base::rank(lfc_ligand, ties.method = "average", na.last = FALSE)),
      scaled_p_val_ligand = base::rank(
        desc(p_val_ligand),
        ties.method = "average",
        na.last = FALSE
      ) /
      ...
      scaled_lfc_pval_ligand = base::rank(
        lfc_pval_ligand,
        ties.method = "average",
        na.last = FALSE
      ) /
      ...
      scaled_p_val_ligand_adapted = base::rank(
...
#'
#'
generate_prioritization_tables_condition_specific_celltypes_receiver = function(
  sender_receiver_info,
  sender_receiver_de,
  ligand_activities_targets_DEgenes,
  contrast_tbl,
...
prioritize_condition_specific_sender <- function(
  ...
  expessed_lr_df_condition_specific_celltypes = abundance_expression_info$sender_receiver_info$frq_df %>%
    ...
    dplyr::select(-n_samples, -n_expressing) %>%
    dplyr::ungroup() %>%
    filter(fraction_expressing_ligand > 0)
...
prioritize_condition_specific_receiver <- function(
  ...
  expessed_lr_df_condition_specific_celltypes = abundance_expression_info$sender_receiver_info$frq_df %>%
    ...
    dplyr::select(-n_samples, -n_expressing) %>%
    dplyr::ungroup() %>%
    filter(fraction_expressing_receptor > 0)
  ...
  ligand_activities_targets_DEgenes_adapted$de_genes_df = bind_rows(
    ...
    cross_join(
      ...
      mutate(gene = NA, logFC = NA, p_val = NA, p_adj = NA) %>%
      select(colnames(
        ligand_activities_targets_DEgenes_adapted$de_genes_df
      )) %>%
      distinct()
