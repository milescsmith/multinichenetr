...
get_muscat_exprs_frac <- function(sce, sample_id, celltype_id, group_id) {
  ...
  celltypes <- sce$cluster_id |>
    unique() |>
    as.character()

  loop_1 <- carrier::crate(function(
    celltype_oi,
    frq,
    all_genes,
    samples,
    ...
    # message(paste("frq type: ", class(frq)))
    # message(paste("frq@assays type: ", class(frq@assays)))
    # message(paste("frq@assays@data type: ", class(frq@assays@data)))
    # message(paste("celltype_oi type: ", class(celltype_oi)))
    # flush.console()
  ...
  frq_lists <-
    # celltypes |>
    # lapply(function(celltype_oi, frq, all_genes) {
    furrr::future_map(
      .x = celltypes,
      .f = loop_1,
      frq = frq,
    ...
    magrittr::set_names(sce$cluster_id |> unique())

  frq_celltype_samples <- frq_lists |>
    purrr::map("frq_celltype_samples") |>
    dplyr::bind_rows()
  frq_celltype_groups <- frq_lists |>
    purrr::map("frq_celltype_groups") |>
    dplyr::bind_rows()
  rm(frq_lists)
...
get_muscat_exprs_avg <- function(sce, sample_id, celltype_id, group_id) {
  ...
  inner_loop <- carrier::crate(
    function(
      ...
      avg_celltype_samples <- avg_celltype_samples |>
        data.frame() |>
        tibble::rownames_to_column("gene") |>
        tidyr::gather(sample, average_sample, -gene) |>
        tibble::as_tibble() |>
        dplyr::mutate(celltype = celltype_oi)
    }
  )

  avg_df <- sce$cluster_id |>
    unique() |>
    # lapply(
    #   function(celltype_oi, avg) {
    furrr::future_map(
      .f = inner_loop,
      avg = avg,
      sce = sce,
...
#' @export
#'
get_pseudobulk_logCPM_exprs <- function(
  sce,
  sample_id,
  celltype_id,
  group_id,
  ...
  if (length(batches) > 1) {
    ...
    warning(
      "You used more than 1 batch/batch to correct for. This is OK for the DE analysis. But, for the Combat batch correction of pseudobulked counts (used in visualization), only the first batch will be considered as batch. If you want to use both as batch, redo the analysis after merging both in one variable."
    )
  } else {
    if (!is.na(batches)) {
      batches_present <- TRUE
  ...
  if (batches_present) {
    ...
    n_combinations_observed <- ei |>
      dplyr::select(group_id, batches) |>
      dplyr::distinct() |>
      nrow()
    n_combinations_possible <- length(levels(ei$group_id)) *
      length(levels(ei$batches))
    if (n_combinations_observed != n_combinations_possible) {
      warning(
        "Not all possible group-batch/batch combinations are present in your data. This will result in errors during the batch effect correction process of Combat and/or Muscat DE analysis. Please reconsider the groups and batches you defined."
      )
    }
    ...
    if (any(table(batch2) <= 1)) {
      warning(
        "For some batch values, only one sample is present in your data. Combat correction of the expression for downstream visualization cannot handle this."
      )
      pb_df <- sce$cluster_id |>
        ...
        furrr::future_map(
          .f = \(celltype_oi) {
            ...
            pseudobulk_counts_celltype <- edgeR::DGEList(pb@assays@data[[
              celltype_oi
            ]])
            ...
            pseudobulk_counts_celltype <- edgeR::calcNormFactors(
              pseudobulk_counts_celltype
            )
      ...
      pb_df <- sce$cluster_id |>
        ...
        furrr::future_map(
          .f = \(celltype_oi) {
            ...
            if (any(table(batch2) <= 1)) {
              warning(paste0(
                "For some batch values for celltype ",
                celltype_oi,
                " only one sample is present in your data. Combat correction of the expression for downstream visualization cannot handle this. Therefore we continue with non-corrected pseudobulk expression values here."
              ))
              adj_count_matrix <- count_matrix
            } else {
              quiet <- function(x) {
                # to suppress sva-combat output
                sink(tempfile())
                on.exit(sink())
                invisible(force(x))
              }
              adj_count_matrix <- quiet(sva::ComBat_seq(
                count_matrix,
                batch = ei$batches,
                group = ei$group_id,
                full_mod = TRUE
...
get_avg_pb_exprs <- function(
  ...
  pb_df <- get_pseudobulk_logCPM_exprs(
    sce,
    sample_id = sample_id,
    celltype_id = celltype_id,
    group_id = group_id,
  ...
  return(list(
    avg_df = avg_df,
    pb_df = pb_df,
    avg_df_group = avg_df_group,
    pb_df_group = pb_df_group
  ))
}
...
get_frac_exprs <- function(
  ...
  print(paste0(
    ...
    fraction_cutoff * 100,
    "% of cells of a cell type of interest in a particular sample will be considered as expressed in that sample."
  ))
  ...
  for (i in seq(length(unique(n_smallest_group_tbl$celltype)))) {
    ...
    print(paste0(
      ...
      n_min,
      " samples will considered as expressed in the cell type: ",
      celltype_oi
    ))
  }
  ...
  for (i in seq(length(unique(expressed_df$celltype)))) {
    ...
    print(paste0(
      n_genes,
      " genes are considered as expressed in the cell type: ",
      celltype_oi
    ))
  }
  return(list(
...
#' @export
#'
get_frac_exprs_sampleAgnostic <- function(
  sce,
  sample_id,
  celltype_id,
  group_id,
  ...
  print(paste0(
    ...
    fraction_cutoff *
      100,
    "% of cells of a cell type of interest in a particular group/condition will be considered as expressed in that group/condition"
  ))
  for (i in seq(length(unique(n_smallest_group_tbl$celltype)))) {
    celltype_oi <- unique(n_smallest_group_tbl$celltype)[i]
    n_min <- n_smallest_group_tbl |>
    ...
    print(paste0(
      "Genes expressed in at least ",
      n_min,
      " group will considered as expressed in the cell type: ",
      celltype_oi
    ))
  }
  frq_df <- frq_df |>
...
#' @export
#'
combine_sender_receiver_info_ic <- function(
  sender_info,
  receiver_info,
  senders_oi,
  receivers_oi,
  ...
  # return
  return(list(
    avg_df = avg_df_sender_receiver,
    frq_df = frq_df_sender_receiver,
    pb_df = pb_df_sender_receiver,
    avg_df_group = avg_df_group_sender_receiver,
    frq_df_group = frq_df_group_sender_receiver,
    pb_df_group = pb_df_group_sender_receiver,
    rel_abundance_df = rel_abundance_df_sender_receiver
  ))
}
...
#' @export
#'
combine_sender_receiver_de <- function(
  sender_de,
  receiver_de,
  senders_oi,
  receivers_oi,
  ...
  de_output_tidy_receiver <- de_output_tidy_receiver |>
    ...
    dplyr::rename(
      receptor = gene,
      lfc_receptor = logFC,
      p_val_receptor = p_val,
      p_adj_receptor = p_adj,
      receiver = cluster_id
    )
