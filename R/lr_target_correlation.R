...
lr_target_prior_cor_inference <- function(
  ...
  lig_rec_send_rec_mapping <- abundance_expression_info$sender_receiver_info$pb_df |>
    dplyr::inner_join(grouping_tbl, by = c("sample", "sender", "receiver")) |>
    dplyr::mutate(
      lr_interaction = paste(ligand, receptor, sep = "_"),
      id = paste(lr_interaction, sender, receiver, sep = "_")
    ) |>
    dplyr::select(sender, receiver, ligand, receptor, id) |>
    dplyr::distinct()

  # make receiver-id mapping to filter later on
  receiver_lr_id_mapping <- abundance_expression_info$sender_receiver_info$pb_df |>
    dplyr::inner_join(grouping_tbl, by = c("sample", "sender", "receiver")) |>
    dplyr::mutate(
      lr_interaction = paste(ligand, receptor, sep = "_"),
      id = paste(lr_interaction, sender, receiver, sep = "_")
    ) |>
    dplyr::select(receiver, sample, id, ligand_receptor_pb_prod) |>
    dplyr::filter(id %in% ids_oi) |>
    dplyr::distinct(receiver, id)
  ...
  lr_prod_df <- abundance_expression_info$sender_receiver_info$pb_df |>
    ...
    dplyr::filter(id %in% ids_oi) |>
    dplyr::distinct() |>
    tidyr::spread(sample, ligand_receptor_pb_prod)
  ...
  col_remove <- lr_prod_mat |>
    apply(2, function(x) sum(x != 0, na.rm = TRUE)) %>%
    .[. == 0] |>
    names()
  ...
  lr_target_cor <- receivers_oi |>
    lapply(function(receiver_oi) {
      ...
      if ("celltype_info" %in% names(abundance_expression_info)) {
        pb_df <- abundance_expression_info$celltype_info$pb_df |>
          dplyr::filter(gene %in% targets_oi & celltype %in% receiver_oi) |>
          dplyr::inner_join(grouping_tbl, by = "sample") |>
          dplyr::filter(keep_receiver == 1 & keep_sender == 1)
      }
      if ("receiver_info" %in% names(abundance_expression_info)) {
        pb_df <- abundance_expression_info$receiver_info$pb_df |>
          dplyr::filter(gene %in% targets_oi & celltype %in% receiver_oi) |>
          dplyr::inner_join(grouping_tbl, by = "sample") |>
          dplyr::filter(keep_receiver == 1 & keep_sender == 1)
      }
      ...
      target_df <- pb_df |>
        dplyr::select(sample, gene, pb_sample) |>
        dplyr::distinct() |>
        tidyr::spread(sample, pb_sample)
      target_mat <- target_df |>
        dplyr::select(-gene) |>
        data.frame() |>
        as.matrix()
      ...
      col_remove <- target_mat |>
        apply(2, function(x) sum(x != 0, na.rm = TRUE)) %>%
        .[. == 0] |>
        names()
      row_remove <- target_mat |>
        apply(1, function(x) sum(x != 0, na.rm = TRUE)) %>%
        .[. == 0] |>
        names()
      ...
      if (length(common_samples) < 5) {
        ...
        cor_df_pearson <- cor_mat$r %>%
          .[, rownames(target_mat)] |>
          data.frame() |>
          tibble::rownames_to_column("id") |>
          tidyr::gather(target, pearson, -id) |>
          tibble::as_tibble()
        cor_df_pearson_pval <- cor_mat$P %>%
          .[, rownames(target_mat)] |>
          data.frame() |>
          tibble::rownames_to_column("id") |>
          tidyr::gather(target, pearson_pval, -id) |>
          tibble::as_tibble()
        ...
        cor_df_spearman <- cor_mat$r %>%
          ...
          data.frame() |>
          tibble::rownames_to_column("id") |>
          tidyr::gather(target, spearman, -id) |>
          tibble::as_tibble()
        cor_df_spearman_pval <- cor_mat$P %>%
          .[, rownames(target_mat)] |>
          data.frame() |>
          tibble::rownames_to_column("id") |>
          tidyr::gather(target, spearman_pval, -id) |>
          tibble::as_tibble()

        cor_df <- lig_rec_send_rec_mapping |>
          dplyr::inner_join(cor_df_pearson, by = "id") |>
          dplyr::inner_join(cor_df_pearson_pval, by = c("id", "target")) |>
          dplyr::inner_join(cor_df_spearman, by = c("id", "target")) |>
          dplyr::inner_join(cor_df_spearman_pval, by = c("id", "target"))
      }
      return(cor_df)
      # print(cor_df)
  ...
  ligand_target_df <- ligand_target_matrix |>
    ...
    tidyr::gather(ligand, prior_score, -target) |>
    tibble::as_tibble()
  ...
  if (nrow(lr_target_cor) > 0) {
    cor_prior_df <- lr_target_cor |>
      dplyr::inner_join(ligand_target_df, by = c("ligand", "target")) |>
      dplyr::mutate(id_target = paste(id, target, sep = "_")) |>
      dplyr::ungroup()
  } else {
    cor_prior_df <- tibble::tibble()
    print(
  ...
  # cor_prior_df = cor_prior_df %>% dplyr::mutate(scaled_cor_score = 0.5*(scaled_pearson + scaled_target_pearson ))
  # cor_prior_df = cor_prior_df %>% dplyr::mutate(final_score = 0.50*(2*scaled_prior_score + scaled_cor_score)) %>% arrange(-final_score)
