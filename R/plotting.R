...
make_sample_lr_prod_activity_plots_Omnipath <- function(
  ...
  omnipath_df <- prioritized_tbl_oi %>%
    distinct(id, curation_effort, n_references, n_resources)
...
make_ggraph_ligand_target_links <- function(
  ...
  plot <- ggraph::ggraph(graph, layout = "dh") +
    ggraph::geom_edge_fan(
      aes(color = direction_regulation),
      edge_width = 1,
  ...
  plot <- suppressWarnings(
    ggraph::ggraph(graph, layout = "nicely") +
      ggraph::geom_edge_link(
        aes(colour = interaction_type, width = weight),
...
make_ligand_activity_target_plot <- function(
  ...
  vis_ligand_target_df <- vis_ligand_target %>%
    ...
    dplyr::inner_join(
      active_ligand_target_links_df %>% distinct(target, direction_regulation)
    ) %>%
