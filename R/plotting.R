...
#'   dplyr::slice_max(n = 50, order_by = prioritization_score)
...
#' @export
#'
make_sample_lr_prod_plots <- function(
  ...
  pb_exprs_data_subset <- pb_exprs_data_subset |>
    dplyr::mutate(
      sender_receiver = factor(
        sender_receiver,
        levels = pb_exprs_data_subset$sender_receiver |> unique()
      )
    )

  keep_sender_receiver_values <- c(0.25, 0.9, 1.75, 4.25)
  names(keep_sender_receiver_values) <- levels(
    pb_exprs_data_subset$keep_sender_receiver
  )
  ...
  p1 <- pb_exprs_data_subset |>
    ...
    ggplot2::theme(
      ...
      axis.text.y = ggplot2::element_text(face = "bold.italic", size = 9),
    ...
    ggplot2::xlab("") +
    ggplot2::ylab("") +
    ggplot2::scale_size_manual(values = keep_sender_receiver_values)
  max_lfc <- abs(pb_exprs_data_subset$scaled_LR_pb_prod) |> max()
  custom_scale_fill <- ggplot2::scale_color_gradientn(
...
#'   dplyr::filter(fraction_expressing_ligand_receptor > 0) |>
#'   dplyr::filter(group == group_oi) |>
#'   dplyr::slice_max(order_by = prioritization_score, n = 50)
...
make_sample_lr_prod_activity_plots <- function(
  ...
  group_data_celltype_specificity <- group_data_celltype_specificity |>
    dplyr::mutate(
      sender_receiver = factor(
        sender_receiver,
        levels = group_data_celltype_specificity$sender_receiver |> unique()
      )
    )
  ...
  group_data <- group_data |>
    dplyr::inner_join(group_data_celltype_specificity) |>
    dplyr::inner_join(group_data_frac_expression)
  group_data <- group_data |>
    dplyr::mutate(
      sender_receiver = factor(
        sender_receiver,
  ...
  p1 <- sample_data |>
    ...
    ggplot2::theme(
      ...
      # axis.title.x = ggplot2::element_text(face = "bold", size = 11),       axis.title.y = ggplot2::element_blank(),
      axis.text.y = ggplot2::element_text(face = "bold.italic", size = 9),
      axis.text.x = ggplot2::element_text(size = 9, angle = 90, hjust = 0),
      panel.grid.major = ggplot2::element_blank(),
      panel.grid.minor = ggplot2::element_blank(),
      panel.spacing.x = grid::unit(0.40, "lines"),
      panel.spacing.y = grid::unit(0.25, "lines"),
  ...
  p2 <- group_data |>
    ggplot2::ggplot(ggplot2::aes(
      direction_regulation,
      lr_interaction,
      fill = activity_scaled
    )) +
    ggplot2::geom_tile(color = "whitesmoke") +
    ggplot2::facet_grid(
      sender_receiver ~ group,
      scales = "free",
      space = "free"
    ...
    ggplot2::theme(
      ...
      panel.grid.major = ggplot2::element_blank(),
      panel.grid.minor = ggplot2::element_blank(),
      panel.spacing.x = grid::unit(0.20, "lines"),
      panel.spacing.y = grid::unit(0.25, "lines"),
      strip.text.x = ggplot2::element_text(
        size = 10,
        color = "black",
  ...
  # cs_data = group_data |> filter(group %in% prioritized_tbl_oi$group) |> dplyr::distinct(sender_receiver, lr_interaction, group, scaled_pb_ligand, scaled_pb_receptor) |> gather(LR, celltype_specificity, scaled_pb_ligand:scaled_pb_receptor)
  cs_data <- group_data |>
    dplyr::distinct(
      sender_receiver,
      lr_interaction,
  ...
  cs_data <- cs_data |> dplyr::inner_join(frac_data)
  ...
  p_cs <- cs_data |>
    ...
    ggplot2::labs(color = "Scaled celltype specificity") +
    ggplot2::labs(size = "Fraction of expression")
...
#'   dplyr::rename(ligand = from, receptor = to) |>
#'   dplyr::distinct(ligand, receptor)
#' lr_network_all <- readRDS("../../../NicheNet_V2/networks/data/ligand_receptor/lr_network_human_allInfo_30112033.rds") |> dplyr::mutate(ligand = convert_alias_to_symbols(ligand, organism = "human"), receptor = convert_alias_to_symbols(receptor, organism = "human"))
#' ligand_target_matrix <- readRDS(url("https://zenodo.org/record/3260758/files/ligand_target_matrix.rds"))
#' sample_id <- "tumor"
#' group_id <- "pEMT"
#' celltype_id <- "celltype"
...
#'   dplyr::filter(group == group_oi) |>
#'   dplyr::slice_max(order_by = prioritization_score, n = 50)
#' plot_oi <- make_sample_lr_prod_activity_plots_Omnipath(output$prioritization_tables, prioritized_tbl_oi |> dplyr::inner_join(lr_network_all))
#' plot_oi
#' }
#'
#' @export
#'
make_sample_lr_prod_activity_plots_Omnipath <- function(
  prioritization_tables,
  prioritized_tbl_oi,
  widths = NULL
) {
  ...
  omnipath_df <- prioritized_tbl_oi |>
    dplyr::distinct(id, curation_effort, n_references, n_resources)
  ...
  group_data_omnipath <- prioritization_tables$group_prioritization_table_source |>
    ...
    dplyr::filter(id %in% sample_data$id) |>
    dplyr::inner_join(omnipath_df) |>
    dplyr::arrange(receiver) |>
    dplyr::group_by(receiver) |>
    dplyr::arrange(sender, .by_group = TRUE)
  group_data_omnipath <- group_data_omnipath |>
    dplyr::mutate(
      sender_receiver = factor(
        sender_receiver,
        levels = group_data$sender_receiver |> unique()
  ...
  group_data <- group_data |> dplyr::inner_join(group_data_omnipath)
  group_data <- group_data |>
    dplyr::mutate(
      sender_receiver = factor(
        sender_receiver,
  ...
  omnipath_data$omnipath_score_type[
    omnipath_data$omnipath_score_type == "curation_effort"
  ] <- "curation effort"
  omnipath_data$omnipath_score_type[
    omnipath_data$omnipath_score_type == "n_references"
  ] <- "nr. references"
  omnipath_data$omnipath_score_type[
    omnipath_data$omnipath_score_type == "n_resources"
  ] <- "nr. resources"
  omnipath_data <- omnipath_data |> dplyr::mutate(omnipath = "Omnipath")
  p_omnipath <- omnipath_data |>
    ...
    ggplot2::labs(size = "Omnipath DB score")
...
#' @export
#'
make_sample_lr_prod_activity_batch_plots <- function(
  prioritization_tables,
  prioritized_tbl_oi,
  grouping_tbl,
  batch_oi,
  ...
  p_batch <- grouping_tbl_plot |>
    ggplot2::ggplot(ggplot2::aes(sample, mock, fill = batch)) +
...
#'   dplyr::distinct(ligand, receiver, group, activity) |>
#'   dplyr::slice_max(order_by = activity, n = 5) |>
#'   dplyr::pull(ligand) |>
#'   unique()
...
#' @export
#'
make_ligand_activity_plots <- function(
  prioritization_tables,
  ligands_oi,
  contrast_tbl,
  widths = NULL
  ...
  group_data <- prioritization_tables$ligand_activities_target_de_tbl |>
    ...
    dplyr::mutate(
      group_direction_regulation = paste(group, direction_regulation, sep = "-")
    )

  p1 <- group_data |>
    ggplot2::ggplot(ggplot2::aes(receiver, ligand, fill = activity_scaled)) +
    ggplot2::geom_tile(color = "whitesmoke") +
    ggplot2::facet_grid(
  ...
  p2 <- group_data |>
    ggplot2::ggplot(ggplot2::aes(receiver, ligand, fill = activity)) +
    ggplot2::geom_tile(color = "whitesmoke") +
    ggplot2::facet_grid(
      . ~ group_direction_regulation,
      scales = "free",
  ...
  max_activity <- (group_data$activity) |> max()
  min_activity <- (group_data$activity) |> min()
  # custom_scale_fill = ggplot2::scale_fill_gradientn(colours = c("white", RColorBrewer::brewer.pal(n = 7, name = "Oranges") |> .[-7]),values = c(0, 0.250, 0.5550, 0.675, 0.80, 0.925, 1),  limits = c(min_activity-0.01, max_activity))
  custom_scale_fill <- ggplot2::scale_fill_gradient2(
    low = "white",
    mid = "white",
    high = "darkorange",
  ...
  if (!is.null(widths)) {
    ...
    p <- patchwork::wrap_plots(
      ...
      guides = "collect",
      widths = c(
        group_data$receiver |> unique() |> length(),
        group_data$receiver |> unique() |> length()
      )
    )
  }
...
#' @export
#'
make_DEgene_dotplot_pseudobulk <- function(
  genes_oi,
  celltype_info,
  prioritization_tables,
  celltype_oi,
  ...
  p2 <- plot_data |>
    # ggplot2::ggplot(ggplot2::aes(gene, sample , fill = scaled_gene_exprs)) +
...
#' @export
#'
make_DEgene_dotplot_pseudobulk_reversed <- function(
  genes_oi,
  celltype_info,
  prioritization_tables,
  celltype_oi,
  ...
  if (is.null(target_regulation_df)) {
    ...
    plot_data <- plot_data |>
      dplyr::group_by(gene, celltype) |>
      dplyr::mutate(
        scaled_gene_exprs = nichenetr::scaling_zscore(pb_sample)
      ) |>
      dplyr::ungroup()
    plot_data$gene <- factor(plot_data$gene, levels = genes_oi)
    ...
    p1 <- plot_data |>
      ...
      ggplot2::theme(
        ...
        strip.text.x = ggplot2::element_text(size = 9, color = "black"),
        strip.background = ggplot2::element_rect(
          color = "darkgrey",
          fill = "whitesmoke",
          size = 1.5,
    ...
    frq_df <- celltype_info$frq_df |>
      dplyr::filter(gene %in% genes_oi & celltype %in% celltype_oi)
    plot_data <- plot_data |>
      dplyr::inner_join(frq_df, by = c("gene", "sample", "celltype"))
    plot_data$gene <- factor(plot_data$gene, levels = genes_oi)
...
#' @export
#'
make_DEgene_dotplot_pseudobulk_batch <- function(
  genes_oi,
  celltype_info,
  prioritization_tables,
  celltype_oi,
  ...
  ### batch plot
  # grouping_tbl = plot_data |> dplyr::distinct(sample, group, batch_oi) |> dplyr::rename(batch = batch_oi)
  # grouping_tbl_plot = grouping_tbl |> dplyr::mutate(batch_ = paste0(" ",batch_oi," "), mock = "")
  grouping_tbl_plot <- grouping_tbl |>
    dplyr::mutate(
      batch_ = paste0(" ", batch_oi, " "),
      mock = "",
  ...
  p_batch <- grouping_tbl_plot |>
    ...
    ggplot2::theme(
      ...
      panel.grid.major = ggplot2::element_blank(),
      panel.grid.minor = ggplot2::element_blank(),
      panel.spacing.x = grid::unit(1.50, "lines"),
      panel.spacing.y = grid::unit(0.25, "lines"),
      strip.text.x = ggplot2::element_text(
        size = 11,
        color = "black",
...
#' @export
#'
make_ligand_receptor_violin_plot <- function(
  sce,
  ligand_oi,
  receptor_oi,
  sender_oi,
  ...
  # ligand plot
  sce_subset <- sce[,
    SummarizedExperiment::colData(sce)[, celltype_id] %in% sender_oi
  ]
  sce_subset <- sce_subset[,
    SummarizedExperiment::colData(sce_subset)[, group_id] %in%
      c(group_oi, background_groups)
  ]

  sce_subset$id <- sce_subset[[sample_id]]
  sce_subset <- muscat::prepSCE(
    sce_subset,
  ...
  exprs_df <- tibble::tibble(
    expression = SingleCellExperiment::logcounts(sce_subset)[ligand_oi, ],
    cell = names(SingleCellExperiment::logcounts(sce_subset)[ligand_oi, ])
  ) |>
    dplyr::inner_join(coldata_df |> tibble::as_tibble())
  ...
  if (is.na(batch_oi)) {
    p_sender <- exprs_df |>
      ...
      ggplot2::geom_violin(color = "grey10") +
      ggbeeswarm::geom_quasirandom(bandwidth = 1.25, varwidth = TRUE) +
      ggplot2::facet_grid(. ~ group_id, scales = "free", space = "free") +
  ...
  # receptor plot
  sce_subset <- sce[,
    SummarizedExperiment::colData(sce)[, celltype_id] %in% receiver_oi
  ]
  sce_subset <- sce_subset[,
    SummarizedExperiment::colData(sce_subset)[, group_id] %in%
      c(group_oi, background_groups)
  ...
  exprs_df <- tibble::tibble(
    expression = SingleCellExperiment::logcounts(sce_subset)[receptor_oi, ],
    cell = names(SingleCellExperiment::logcounts(sce_subset)[receptor_oi, ])
  ) |>
    dplyr::inner_join(coldata_df |> tibble::as_tibble())
...
#' @export
#'
make_target_violin_plot <- function(
  sce,
  target_oi,
  receiver_oi,
  group_oi,
  ...
  exprs_df <- tibble::tibble(
    expression = SingleCellExperiment::logcounts(sce_subset)[target_oi, ],
    cell = names(SingleCellExperiment::logcounts(sce_subset)[target_oi, ])
  ) |>
    dplyr::inner_join(coldata_df |> tibble::as_tibble())
...
#' @export
#'
make_lr_target_scatter_plot <- function(
  prioritization_tables,
  ligand_oi,
  receptor_oi,
  sender_oi,
  ...
  p <- ligand_receptor_target_pb_df |>
    ggplot2::ggplot(ggplot2::aes(ligand_receptor_pb_prod, target_pb)) +
    ggplot2::geom_point(ggplot2::aes(color = group), size = 2) +
    ggplot2::geom_smooth(method = "lm", alpha = 0.10, color = "grey50") +
    ...
    ggplot2::ggtitle(paste(
      ...
      "-",
      receptor_oi,
      " expression vs target expression between ",
      sender_oi,
      " and ",
      receiver_oi,
      " cells.",
...
#' @export
#'
make_lr_target_prior_cor_heatmap <- function(
  lr_target_prior_cor_filtered,
  add_grid = TRUE
) {
  requireNamespace("dplyr")
  ...
  if (add_grid == TRUE) {
    p_lr_target <- lr_target_prior_cor_filtered |>
      ...
      ggplot2::ggplot(ggplot2::aes(
        ...
        color = prior_score,
        size = pearson
      )) + # alternative to scaled_prior_score: scaled_ligand or prior_score
      ggplot2::geom_point() +
      ggplot2::facet_grid(
        sender_receiver ~ target,
        scales = "free",
        space = "free"
      ) +
      ggplot2::scale_x_discrete(position = "top") +
      ...
      ggplot2::theme(
        ...
        panel.grid.major = ggplot2::element_blank(),
        panel.grid.minor = ggplot2::element_blank(),
        panel.spacing.x = grid::unit(0.175, "lines"),
        panel.spacing.y = grid::unit(0.25, "lines"),
        strip.text.x.top = ggplot2::element_text(
          size = 0,
          color = "black",
      ...
      ggplot2::labs(color = "Prior knowledge score\nof correlated targets") +
      ggplot2::labs(
  ...
  custom_scale_color <- ggplot2::scale_color_gradientn(
    colours = c(
      "white",
      RColorBrewer::brewer.pal(n = 11, name = "PiYG") |> .[1:5] |> rev()
    ),
    values = c(0, 0.025, 0.075, 0.25, 0.40, 0.55, 1),
    limits = c(0, max(lr_target_prior_cor_filtered$prior_score))
  )

  # custom_scale_color = ggplot2::scale_color_gradientn(colours = c("white", RColorBrewer::brewer.pal(n = 7, name = "BuPu")) ,values = c(0, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1),  limits = c(0, 1))
...
#' receiver_oi <- "Malignant"
#' prioritized_tbl_oi <- output$prioritization_tables$group_prioritization_tbl |>
#'   dplyr::distinct(id, ligand, receptor, sender, receiver, lr_interaction, group, ligand_receptor_lfc_avg, activity_scaled, fraction_expressing_ligand_receptor, prioritization_score) |>
...
#'   dplyr::slice_max(order_by = prioritization_score, n = 250)
#' lr_target_prior_cor_filtered <- output$lr_target_prior_cor |> filter(scaled_prior_score > 0.50 & (pearson > 0.66 | spearman > 0.66))
...
#'   dplyr::slice_max(order_by = prioritization_score, n = 2)
...
#' @export
#'
make_lr_target_correlation_plot <- function(
  prioritization_tables,
  prioritized_tbl_oi,
  lr_target_prior_cor_filtered,
  grouping_tbl,
  ...
  p2 <- lr_target_data |>
    ...
    ggplot2::theme(
      ...
      panel.grid.major = ggplot2::element_blank(),
      panel.grid.minor = ggplot2::element_blank(),
      panel.spacing.x = grid::unit(0.50, "lines"),
      panel.spacing.y = grid::unit(0.25, "lines"),
      strip.text.x.top = ggplot2::element_text(
        size = 9,
        color = "black",
  ...
  custom_scale_color <- ggplot2::scale_color_gradientn(
    ...
    values = c(0, 0.0125, 0.0040, 0.175, 0.35, 0.50, 1),
    limits = c(0, max(lr_target_prior_cor_filtered$prior_score))
  )
  # custom_scale_fill = ggplot2::scale_fill_gradientn(colours = c("white", RColorBrewer::brewer.pal(n = 7, name = "BuPu")) ,values = c(0, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1),  limits = c(0, 1))
  ...
  p_target_exprs <- make_DEgene_dotplot_pseudobulk_reversed(
  ...
  legends <- patchwork::wrap_plots(
    ggpubr::as_ggplot(ggpubr::get_legend(p_lr_exprs)),
  ...
  if (plot_legend == FALSE) {
    ...
    combined_plot <- patchwork::wrap_plots(
      ...
      B = p_lr_target +
        ...
        ggplot2::ylab(""),
      C = p_target_exprs + ggplot2::theme(legend.position = "none"),
      nrow = 2,
      design = design,
      widths = widths,
      heights = heights
...
#' @export
#'
make_ggraph_ligand_target_links <- function(
  lr_target_prior_cor_filtered,
  prioritized_tbl_oi,
  colors
) {
  ...
  network <- igraph::graph_from_data_frame(
    d = links |> dplyr::filter(type == "Ligand-Target"),
    vertices = nodes,
    directed = T
  )
  ...
  plot <- ggraph::ggraph(graph, layout = "dh") +
    ggraph::geom_edge_fan(
      ggplot2::aes(color = direction_regulation),
      edge_width = 1,
      arrow = arrow(length = grid::unit(3, "mm")),
      end_cap = ggraph::circle(5.5, "mm"),
      start_cap = ggraph::circle(3, "mm")
    ) +
    # ggraph::geom_edge_loop(ggplot2::aes(color = direction_regulation), edge_width = 1, alpha = 0.70)  +
    ggraph::geom_node_label(
      ggplot2::aes(label = gene, fill = celltype),
      fontface = "bold",
      size = 3.5,
      nudge_x = 0,
      nudge_y = 0,
  ...
  return(list(
    plot = plot,
    graph = graph,
    source_df_lr = source_df_lr,
    source_df_lt = links |> dplyr::filter(type == "Ligand-Target"),
    nodes_df = nodes
  ))
}
...
#'   dplyr::group_by(ligand, sender, group) |>
#'   dplyr::slice_max(order_by = prioritization_score, n = 2)
#' lr_target_df <- lr_target_prior_cor_filtered |> dplyr::distinct(group, sender, receiver, ligand, receptor, id, target, direction_regulation)
...
visualize_network <- function(network, colors) {
  ...
  network_igraph <- igraph::graph_from_data_frame(
    d = network$links |> dplyr::filter(type == "Ligand-Target"),
    vertices = nodes,
    directed = T
  )
...
#' active_signaling_network_min_max$sig = active_signaling_network_min_max$sig |> dplyr::mutate(weight = ((weight-min(weight))/(max(weight)-min(weight))) + 0.75)
#' active_signaling_network_min_max$gr = active_signaling_network_min_max$gr |> dplyr::mutate(weight = ((weight-min(weight))/(max(weight)-min(weight))) + 0.75)
...
#' @export
#'
make_ggraph_signaling_path <- function(
  ...
  edge_df <- dplyr::bind_rows(
    ...
    signaling_graph_list$gr |>
      dplyr::mutate(interaction_type = "gene regulatory")
  )

  nodes <- union(edge_df$from, edge_df$to)
  node2type <- rep("mediator", times = length(nodes)) |>
    magrittr::set_names(nodes)
  ...
  nodes_df <- tibble::tibble(node = nodes) |>
    dplyr::mutate(node_type = node2type[node])
  ...
  plot <- suppressWarnings(
    ggraph::ggraph(graph, layout = "nicely") +
      ggraph::geom_edge_link(
        ggplot2::aes(colour = interaction_type, width = weight),
        arrow = arrow(length = grid::unit(4, "mm")),
        end_cap = ggraph::circle(5.5, "mm"),
        start_cap = ggraph::circle(3, "mm")
      ) +
      ggraph::geom_node_label(
        ggplot2::aes(label = name, color = node_type),
        fontface = "bold",
        size = 4.5,
        nudge_x = 0,
        nudge_y = 0
      ) +
      ggraph::theme_graph(foreground = "black", fg_text_colour = "white") +
...
#'   dplyr::filter(group == group_oi & receiver == receiver_oi) |>
#'   dplyr::slice_max(order_by = prioritization_score, n = 50) |>
#'   dplyr::slice_max(order_by = activity_scaled, n = 25) |>
#'   dplyr::arrange(-activity_scaled)
...
#' @export
#'
make_ligand_activity_target_plot <- function(
  group_oi,
  receiver_oi,
  prioritized_tbl_oi,
  prioritization_tables,
  ...
  order_ligands_ <- generics::intersect(
    best_upstream_ligands,
  ...
  p_ligand_target_network <- vis_ligand_target_df |>
    ggplot2::ggplot(ggplot2::aes(target, ligand, fill = score)) +
    ggplot2::geom_tile(color = "whitesmoke", size = 0.5) +
    ggplot2::facet_grid(
      . ~ direction_regulation,
      scales = "free",
      space = "free"
    ...
    ggplot2::labs(fill = "Regulatory Potential") +
    ggplot2::xlab("Predicted target genes") +
    ggplot2::ylab("Prioritized ligands")

  # custom_scale_fill = ggplot2::scale_fill_gradientn(colours = c("white", "plum1", "orchid2","orchid4","violetred"),values = c(0, 0.05, 0.50, 0.80, 1),  limits = c(0, max(ligand_target_matrix)))
  # custom_scale_fill = ggplot2::scale_fill_gradientn(colours = c("white", RColorBrewer::brewer.pal(n = 11, name = "PiYG") |> .[1:5] |> rev()),values = c(0, 0.025, 0.075, 0.25, 0.40, 0.55, 1),  limits = c(0, max(ligand_target_matrix)))
  custom_scale_fill <- ggplot2::scale_fill_gradientn(
    colours = c(
      "white",
      RColorBrewer::brewer.pal(n = 11, name = "PiYG") |> .[1:5] |> rev()
    ),
    values = c(0, 0.04, 0.12, 0.30, 0.40, 0.55, 1),
  ...
  p_ligand_activity_scaled <- ligand_activity_df |>
    # ggplot2::ggplot(ggplot2::aes(receiver, lr_interaction, color = activity_scaled, size = activity)) +
    # geom_point() +
    ggplot2::ggplot(ggplot2::aes(
      direction_regulation,
      ligand,
  ...
  p_ligand_activity <- ligand_activity_df |>
    ...
    ggplot2::theme(
      ...
      panel.grid.minor = ggplot2::element_blank(),
      panel.spacing.x = grid::unit(0.50, "lines"),
      panel.spacing.y = grid::unit(0.50, "lines"),
      strip.text.x = ggplot2::element_text(size = 10, color = "black"),
      strip.text.y = ggplot2::element_blank(),
      strip.background = ggplot2::element_rect(
        color = "darkgrey",
    ...
    ggplot2::labs(fill = "Ligand Activity\nin Receiver") +
    ggplot2::ylab("Prioritized ligands") +
  ...
  p_targets <- make_DEgene_dotplot_pseudobulk_reversed(
    genes_oi = order_targets_,
    celltype_info = receiver_info,
    prioritization_tables = prioritization_tables,
    celltype_oi = receiver_oi,
    grouping_tbl = grouping_tbl,
  ...
  legends <- patchwork::wrap_plots(
    ...
    patchwork::wrap_plots(ggpubr::as_ggplot(ggpubr::get_legend(
      p_targets$pseudobulk_plot
    )))
...
#' #  dplyr::distinct(id, sender, receiver, ligand, receptor, group, prioritization_score, ligand_receptor_lfc_avg, fraction_expressing_ligand_receptor) |>
#' #  dplyr::filter(ligand_receptor_lfc_avg > 0 & fraction_expressing_ligand_receptor > 0) |> dplyr::slice_max(order_by = prioritization_score, n = 30)
# prioritized_tbl_oi = output$prioritization_tables$group_prioritization_tbl |>
#' #  dplyr::filter(id %in% prioritized_tbl_oi_prep$id) |>
#' #  dplyr::distinct(id, sender, receiver, ligand, receptor, group) |> dplyr::left_join(prioritized_tbl_oi_prep)
...
#' @export
#'
make_circos_group_comparison <- function(
  prioritized_tbl_oi,
  colors_sender,
  colors_receiver
) {
  ...
  all_plots <- groups_oi |>
    lapply(function(group_oi) {
      ...
      if (
        length(gaps) !=
          length(union(circos_links$ligand, circos_links$receptor) |> unique())
      ) {
        warning(
...
#'
# group_oi = "High"
# prioritized_tbl_oi = output$prioritization_tables$group_prioritization_tbl |> filter(ligand_receptor_lfc_avg > 0 & fraction_expressing_ligand_receptor > 0 & group == group_oi) |> dplyr::slice_max(order_by = prioritization_score, n = 25)
# senders_receivers = union(prioritized_tbl_oi$sender |> unique(), prioritized_tbl_oi$receiver |> unique())
# colors_sender = RColorBrewer::brewer.pal(n = length(senders_receivers), name = 'Spectral') |> magrittr::set_names(senders_receivers)
# colors_receiver = RColorBrewer::brewer.pal(n = length(senders_receivers), name = 'Spectral') |> magrittr::set_names(senders_receivers)
...
#' @export
#'
make_circos_one_group <- function(
  prioritized_tbl_oi,
  colors_sender,
  colors_receiver
) {
...
compare_normal_emp_pvals <- function(DE_info, DE_info_emp, adj_pval = FALSE) {
  ...
  comparison_plots <- DE_info$celltype_de$de_output_tidy$cluster_id |>
    unique() |>
    lapply(
      function(celltype_oi, adjusted = FALSE) {
        if (adjusted == TRUE) {
        ...
        upset_df <- tibble::tibble(
          gene = union(de_genes_normal, de_genes_emp),
          normal = as.double(gene %in% de_genes_normal),
          empirical = as.double(gene %in% de_genes_emp)
        ) |>
          data.frame() |>
        ...
        p_ranking <- DE_info_emp$de_output_tidy_emp |>
          dplyr::filter(
            gene %in%
              union(de_genes_normal, de_genes_emp) &
          ...
          ggplot2::ggplot(ggplot2::aes(
            ...
            rank,
            group = gene,
            color = empirical_lower
          )) +
          ggplot2::geom_line(ggplot2::aes(group = gene)) +
          ggplot2::facet_grid(cluster_id ~ contrast) +
          ggplot2::theme_bw()
